...
void STP23_data_deal(const std::vector<uint8_t>& buffer, rclcpp::Node::SharedPtr nh) {
    if (buffer.size() != 10 || buffer.back() != 0x7D) {
        RCLCPP_WARN(rclcpp::get_logger("serial_listener"), "Invalid STP23 frame");
        return;
    }

    s21c_receive_msg::msg::STP23 msg;
    msg.distance_1 = (buffer[0] << 8) | buffer[1];
    msg.distance_2 = (buffer[2] << 8) | buffer[3];
    msg.distance_3 = (buffer[4] << 8) | buffer[5];
    msg.distance_4 = (buffer[6] << 8) | buffer[7];

    static auto publisher = nh->create_publisher<s21c_receive_msg::msg::STP23>("STP23", 10);
...
int main(int argc, char **argv) {
    ...
    // 设置串口参数
    nh->declare_parameter<std::string>("port", "/dev/ttyACM0");
    const std::string port = nh->get_parameter("port").as_string();
        ...
        if (ser.available()) {
            ...
            if (bytes_read == 1) {
                if (module_n == 2) {
                    ...
                    if (module_n == 1) {
                        // 实际 STP23 帧无 0x7B 帧头：4 距离 + 校验字节 + 0x7D。
                        // 串口还夹有调试文本，遇到 0x7D 时截取末尾 10 字节。
                        buffer.push_back(byte);
                        if (byte == 0x7D && buffer.size() >= 10) {
                            std::vector<uint8_t> frame(buffer.end() - 10, buffer.end());
                            STP23_data_deal(frame, nh);
                            buffer.clear();
                        } else if (buffer.size() > 512) {
                            buffer.erase(buffer.begin(), buffer.end() - 10);
                        }
                    } else if (!frame_start_found) {
